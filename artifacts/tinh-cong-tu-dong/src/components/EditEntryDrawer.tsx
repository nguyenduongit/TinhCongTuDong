...
import { useUpdateSanLuong, type SanLuong, type CongDoan } from '@workspace/api-client-react';
...
interface CongDoanBlock {
  id: string;
  congDoan: { ma_cong_doan: string } | null;
  soLuong: string;
  phanTram: string;
}
...
export function EditEntryDrawer({ entry, open, onOpenChange }: EditEntryDrawerProps) {
  ...
  useEffect(() => {
    if (entry && open) {
      ...
      setCongDoanBlocks(entry.chi_tiet.map((ct, idx) => ({
        id: idx.toString(),
        congDoan: { ma_cong_doan: ct.cong_doan },
  ...
  const handleAddBlock = () => {
    setCongDoanBlocks([...congDoanBlocks, { id: Date.now().toString(), congDoan: null, soLuong: '', phanTram: '100%' }]);
  };
  ...
  const handleSelectCongDoan = (congDoan: CongDoan) => {
    if (modalTargetIndex !== null) {
      ...
      updatedBlocks[modalTargetIndex].congDoan = { ma_cong_doan: congDoan.ma_cong_doan };
        ...
        <Drawer.Portal>
          ...
          <Drawer.Content className="bg-background border-t border-border flex flex-col rounded-t-[2rem] h-[85vh] mt-24 fixed bottom-0 left-0 right-0 z-50 mx-auto max-w-[430px] outline-none before:absolute before:top-0 before:left-0 before:right-0 before:h-24 before:bg-gradient-to-b before:from-primary/5 before:to-transparent before:pointer-events-none">
            <div className="p-4 bg-background rounded-t-[2rem] flex-1 flex flex-col relative">
              ...
              <Drawer.Title className="text-2xl font-bold text-white mb-6 px-2 tracking-tight">Cập nhật sản lượng</Drawer.Title>
